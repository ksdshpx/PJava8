...
public class TestStream4 {
    public static void main(String[] args) {
        ...
        list2.stream().map((x) -> x * x).collect(Collectors.toList()).forEach(System.out::println);
        System.out.println("------------");
        Stream<List<Integer>> stream = Stream.of(Arrays.asList(1), Arrays.asList(2, 3), Arrays.asList(4, 5, 6));
        stream.flatMap(theList -> theList.stream()).map(x -> x * x).collect(Collectors.toList()).forEach(System.out::println);
    }
}
